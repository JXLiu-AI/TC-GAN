...
class TrainDatasetFromFolder(Dataset):
    ...
    def __len__(self):
        print(len(self.visible_image_filenames))
...
class TestDatasetFromFolder(Dataset):
    def __init__(self, dataset_dir, upscale_factor):
        ...
        self.infrared_image_filenames = [x.replace('tmp/vi','tmp/ir') for x in self.visible_image_filenames]
        #self.infrared_image_filenames = [join(dataset_dir+'tmp/ir/', x) for x in in imagelist if is_image_file(x)]
    def __getitem__(self, index):
        visible_image = Image.open(self.visible_image_filenames[index])
        infrared_image = Image.open(self.infrared_image_filenames[index])
