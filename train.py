...
CROP_SIZE = opt.crop_size  # Crop size for training images
UPSCALE_FACTOR = opt.upscale_factor  # Upscale factor
NUM_EPOCHS = opt.num_epochs  # Number of epochs

train_set = TrainDatasetFromFolder('E:/Dataset/', crop_size=CROP_SIZE, upscale_factor=UPSCALE_FACTOR) # load training dataset
val_set = ValDatasetFromFolder('E:/Dataset/', upscale_factor=UPSCALE_FACTOR) # load validation dataset
train_loader = DataLoader(dataset=train_set, num_workers=0, batch_size=64, shuffle=True) # prepare training DataLoader
val_loader = DataLoader(dataset=val_set, num_workers=0, batch_size=1, shuffle=False)

netG = Generator(UPSCALE_FACTOR)  # Network model
...
generator_criterion = GeneratorLoss(batchSize=64) # generator loss
...
for epoch in range(1, NUM_EPOCHS + 1):
    ...
    for data, ir, target in train_bar:
        ...
        running_results['batch_sizes'] += batch_size # batch sizes in PyTorch may vary
        ...
        if torch.cuda.is_available():
            z = z.cuda()
        fake_img = netG(z) # generate image

        netD.zero_grad()
        real_out = netD(real_img) # discriminator predicts real
        fake_out = netD(fake_img) # discriminator predicts fake
        ...
        g_loss, adversrial_loss, img_loss, tv_loss, d_perception_loss = generator_criterion(fake_out, fake_img, real_img, target_ir, lamda_d_loss_fake, lamda_d_loss_real)  # Calculate G_loss twice
        ...
        count += 1
    netG.eval()
    out_path = '/data/ResnetFusion/training_results/SRF_' + str(UPSCALE_FACTOR) + '/' # output path
    if not os.path.exists(out_path):
        os.makedirs(out_path)
    val_bar = tqdm(val_loader) # validation progress bar
    ...
    for val_lr , val_lr_restore, val_hr in val_bar:
        ...
        with torch.no_grad():
            ...
            if torch.cuda.is_available():
                lr = lr.cuda()
                hr = hr.cuda()
            sr = netG(lr)  # Generate validation image

        val_images.extend(
            [display_transform()(val_lr_restore.squeeze(0)), display_transform()(hr.data.cpu().squeeze(0)),
             display_transform()(sr.data.cpu().squeeze(0))])
    val_images = torch.stack(val_images)
    val_images = torch.chunk(val_images, 1) # trick: chunk images (e.g. val_images.size(0) // 15)
    val_save_bar = tqdm(val_images, desc='[saving training results]')
    index = 1
    for image in val_save_bar:
        image = utils.make_grid(image, nrow=3, padding=5)
        utils.save_image(image, out_path + 'epoch_%d_index_%d.png' % (epoch, index), padding=5)  # Save validation results
        index += 1

    # save model parameters
    torch.save(netG.state_dict(), 'E:/Project/NewProject/ResNetFusion-master/model1/netG_epoch_%d_%d.pth' % (UPSCALE_FACTOR, epoch))  # Save network parameters
    torch.save(netD.state_dict(), 'E:/Project/NewProject/ResNetFusion-master/model1/netD_epoch_%d_%d.pth' % (UPSCALE_FACTOR, epoch))
